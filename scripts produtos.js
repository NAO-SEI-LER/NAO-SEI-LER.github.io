...
function carregarListaP() {
	...
	xhttp.onreadystatechange = function() {
  	...
  	xhttp.open("POST", "produtos.php", true);
...
function carregarProduto( obj ) {
	...
	xhttp.onreadystatechange = function() {
    	if (this.readyState == 4 && this.status == 200) {
      		...
      		document.getElementById('preço').value 		= resultado[0].preço;
      		document.getElementById('quantidade').value 	= resultado[0].quantidade;
...
function salvarFormP() {
	...
	xhttp.onreadystatechange = function() {
    	if (this.readyState == 4 && this.status == 200) {
    		//limpa o formulario
    		restauraForm();
    		document.getElementById('frmProdutos').reset();
    		//exibe mensagem de sucesso na tela por alguns segundos
      		document.getElementById('msg-php').innerHTML = this.responseText;
      		document.getElementById('msg-php').classList.remove("no-display");
      		document.getElementById('msg-php').classList.add("msg-php");
  	...
  	formData.append("preço", document.getElementById("preço").value);
  	...
  	//xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  	xhttp.open("POST", "produtos.php?action=salvar", true);
  	xhttp.send( formData );
}
